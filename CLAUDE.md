...
**Hito 0 complete — the stack is runnable** (`docker compose up` → postgres/temporal/redis/minio/api/worker; the create→version→launch→execute vertical works end-to-end against real Temporal; see `README.md`). Built so far: workflow validation core, Temporal interpreter (command/rest), persistence (docs-03 schema + Alembic), workflow CRUD + versioning, executions (idempotent launch), JWT auth + RBAC, docker-compose, CI, live execution feed (worker→Redis→API subscriber persists `execution_events` + syncs status; `GET /executions/{id}/events` + WebSocket `/executions/{id}/stream`), and the **frontend UI** (`services/frontend/`, Vite+React+TS+React Flow: login, workflow authoring with live DAG preview, launch, live execution monitor). Since then, also built: `sql` job, cron triggers (Temporal Schedules), live agent (`run_on: agent`, local), `simulate` + machine-readable catalogs, management endpoints (users CRUD, workflow delete/export), `job_executions` (per-job projection) + `cancel` + `audit_log`, **artifacts in MinIO** (`with.artifacts` → upload → presigned download), **command isolation** (non-root + rlimits + ephemeral workdir), and **Hito 5 — the secure remote agent** (slices 1-5: enroll/register/approve/revoke lifecycle, internal CA + cert issuance, per-agent envelope encryption (ADR-0013), revocation enforcement via `POST /agents/verify`, and the admin Agents UI). Still to build: `job_executions` per-*attempt* rows, replay UI, webhook trigger, and **real mTLS agent↔Temporal transport** (slice 4 left it as config plumbing — the dev server can't verify it).

### Frontend (`services/frontend/`)
Vite + React + TS. Typed client (`src/api.ts`) mirrors the OpenAPI; `src/auth.tsx` holds JWT + RBAC helpers. Run: `npm install && npm run dev` (port 5173, reads `VITE_API_URL`). Build/verify: `npm run build` (tsc + vite). The API enables CORS for the SPA origin (`CORS_ORIGINS`). Dark "loom of the Fates" theme in `src/styles.css`. The docs are Spanish; the **product/UI/code is English** (i18n-ready). Treat the docs as the source of truth (design refined in the v2 ADRs — see `docs/00-README.md` changelog). Read them before scaffolding new subsystems:

...
async def run_dag(
    ...
    results only through declared outputs.
    """
    ...
    while len(completed) < len(jobs):
        ...
        results = await asyncio.gather(*(_execute(job, scope, run_job) for job in batch))
...
async def _execute(job: dict[str, Any], scope: RenderScope, run_job: RunJob) -> JobResult:
